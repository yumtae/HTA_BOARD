               ...
               $(rdata.boardlist).each(function(){
                  ...
                  if($("#loginid").val()==this.id){
                     output += '<div class="comment-tool">'
                           ...
                           +  '     <div>&#46;&#46;&#46;</div>'
                           +  ' </div>'
                           +  ' <div id ="comment-list-item-layer' + this.num + '"  class="LayerMore">' //스타일에서 disply:none; 설정
                           +  '  <ul class="layer-list">'
                           +  '    <li class="layer-item">'
...
$(function() {
   ...
   $('.comment-area').on('click','.btn-cancel',function(){
      ...
      //수정 폼이 있는 상태에서 더보기를 클릭할 수 없도록 더 보기 여역을 숨겼는데 취소를 선택하면 보여주도록 합니다.
      $(".comment-tool").show();
   })//수정 후 취소 버튼을 클릭한 경우
